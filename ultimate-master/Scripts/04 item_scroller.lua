-- SM5.2 fallback item_scroller compatibility for Ultimate on PMOD.
-- Ultimate's custom select music menus use this helper instead of ActorScroller.
...
local function check_metatable(item_metatable)
	assert(item_metatable.__index.create_actors, "The metatable must have a create_actors function.")
	assert(item_metatable.__index.transform, "The metatable must have a transform function.")
	assert(item_metatable.__index.set, "The metatable must have a set function.")
end
...
local function internal_scroll(self, start_pos)
	...
	if math.abs(shift_amount) < #self.items then
		...
		if shift_amount < 0 then
			for n= 1, math.abs(shift_amount)+1 do
				if self.items[n] then
					call_item_set(self, n, self:maybe_wrap_index(self.info_pos, n, self.info_set))
				end
			end
		...
		for i= 1, #self.items do
			call_item_set(self, i, self:maybe_wrap_index(self.info_pos, i, self.info_set))
		end
	end
	...
	__index= {
		create_actors= function(self, name, num_items, item_metatable, mx, my, item_params)
			...
			for n= 1, num_items do
				...
				args[#args+1]= item:create_actors(sub_params)
			end
		...
		set_info_set= function(self, info, pos)
			...
			for n= 1, #self.items do
				call_item_set(self, n, self:maybe_wrap_index(start_pos, n, info))
			end
		...
		scroll_to_pos= function(self, pos)
			internal_scroll(self, calc_start(self, self.info_set, pos))
		end,
		scroll_by_amount= function(self, amount)
			internal_scroll(self, self.info_pos + amount)
		end,
